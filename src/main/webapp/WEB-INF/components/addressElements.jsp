...
<div class="form-label-group">
    <jsp:include page="labeledInput.jsp">
        ...
        <jsp:param name="ERROR_MESSAGE" value="Je třeba zadat poštovní směrovací číslo. (např 31200)"/>
...
<div class="form-group">
    ...
    <select class="form-control" id="<%= FormConfig.STATE%>"
        ...
        <option value="CZ" <c:if test="${customer.contactInfo.address.state.CZ == true}">selected</c:if>>Česká Republika
        </option>
        <option value="SVK" <c:if test="${customer.contactInfo.address.state.SVK == true}">selected</c:if>>Slovenská Republika
        </option>
